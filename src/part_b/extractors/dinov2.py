"""Part B comparison feature: DINOv2 embedding of the face image (a general image model).

The same `facebook/dinov2-base` used in Part A, here applied to the face crops as a control —
answers "does the face-specialized ArcFace cluster faces better than a general backbone?".
Embeds each face JPG directly (CLS token); exposes no attributes.
"""
...
class DINOv2FaceExtractor:
    ...
    def extract(self, items: Sequence[Asset]) -> Embeddings:
        ...
        for asset in items:
            ...
            ids.append(asset.id)
        if not vecs:
            raise ValueError("DINOv2 (faces) produced no embeddings (no readable images)")
